function newgroundsPlugin(k) {
	const newgrounds = {
		ngInit(app_id, cipher = 0, debug = 0) {
			...
			const scoreboardResult = k.ngCall("ScoreBoard.getBoards", 0, 0);
			k.scoreboards = scoreboardResult?.result?.data?.scoreboards ?? [];

			const resultMedals = k.ngCall("Medal.getList", 0, 0);
		...
		ngPostScore(id, value) {
			if (!k.scoreboards || !k.scoreboards.find((b) => b.id == id))
				return;
		...
		ngGetScores(id, user = 0, social = 0, skip = 0, limit = 10) {
			...
			ngCall(
				...
				{ id: board.id, user, social, skip, limit },
				0
			);
		},
		...
		ngEncryptCall(call) {
			...
			const encrypted = k.CryptoJS.AES.encrypt(
				JSON.stringify(call),
				aesKey,
				{ iv }
			);
			const secure = k.CryptoJS.enc.Base64.stringify(
	...
	!(function (t, e) {
		...
		var t =
			...
			(function (t, e) {
				...
				(c.BufferedBlockAlgorithm = s.extend({
					reset: function () {
					...
					_process: function (e) {
						var r,
							...
							f =
								(s = e
									? t.ceil(s)
									: t.max((0 | s) - this._minBufferSize, 0)) *
								c,
							u = t.min(4 * f, o);
						if (f) {
							for (var h = 0; h < f; h += c)
								this._doProcessBlock(n, h);
							(r = n.splice(0, f)), (i.sigBytes -= u);
						}
								...
								for (
									...
									c < r
										...
										: ((f = o[c - 1]),
										  ...
										  (o[c] = o[c - r] ^ f));
								for (
									var s = (this._invKeySchedule = []), a = 0;
									a < i;
									a++
								) {
									c = i - a;
